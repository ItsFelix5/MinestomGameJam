...
class BlockBreakManager {
    ...
    private boolean shouldPreventBreaking(@NotNull Player player, Block block) {
        if (player.getGameMode() == GameMode.SPECTATOR) {
            ...
            if (!itemInMainHand.has(ItemComponent.CAN_BREAK)) return false;
            return !itemInMainHand.get(ItemComponent.CAN_BREAK).test(block);
        }
    ...
    private double getBlockBreakingSpeed(Player player, Block block) {
        ...
        if (breakingSpeed > 1) {
            int efficiency = 0;
            if (stack.has(ItemComponent.ENCHANTMENTS)) {
                EnchantmentList enchantments = stack.get(ItemComponent.ENCHANTMENTS);
                if (enchantments.has(Enchantment.EFFICIENCY)) efficiency = enchantments.level(Enchantment.EFFICIENCY);
            }
            if (efficiency > 0 && !stack.isAir()) breakingSpeed += efficiency * efficiency + 1;
        }

        byte haste = -1;
        byte miningFatigue = -1;
        for (TimedPotion potion : player.getActiveEffects()) {
            if (potion.potion().effect() == PotionEffect.HASTE) {
                haste = potion.potion().amplifier();
            } else if (potion.potion().effect() == PotionEffect.MINING_FATIGUE) {
                miningFatigue = potion.potion().amplifier();
            }
        }
    ...
    private boolean isItemEffective(Material material, Block block) {
        ...
        if (material.name().contains("shears"))
            return block.compare(Block.COBWEB) || block.compare(Block.REDSTONE_WIRE) || block.compare(Block.TRIPWIRE);
